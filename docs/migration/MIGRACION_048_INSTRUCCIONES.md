# 🚨 MIGRACIÓN PENDIENTE: Cuentas Socios

## Problema
La tabla `partner_accounts` no existe en la base de datos de producción, causando error 500 al intentar crear socios.

## Solución
Ejecutar la migración 048 manualmente en Supabase SQL Editor.
...
1. **Abre el SQL Editor:**
   - Ve a: https://supabase.com/dashboard/project/pmqvplyyxiobkllapgjp/sql/new
   - O ve a tu proyecto → SQL Editor → New Query
...
3. **Ejecuta el SQL:**
   ...
   - Espera a ver "Success" o "Success. No rows returned"

4. **¡Listo!** Las tablas estarán creadas y podrás crear socios.

### Opción 2: Script Node.js (Si tienes acceso a variables de entorno)

```bash
npx tsx scripts/run-partner-accounts-migration.ts
```

**Nota:** Este script puede requerir ajustes dependiendo de tu configuración de Supabase.
...
WHERE table_schema = 'public' 
  AND table_name IN ('partner_accounts', 'partner_withdrawals');
```

Deberías ver ambas tablas en el resultado.
...
Esta migración crea:
- ✅ Tabla `partner_accounts` - Para gestionar socios
- ✅ Tabla `partner_withdrawals` - Para registrar retiros
- ✅ Índices para optimizar consultas
- ✅ Relaciones con otras tablas (users, financial_accounts, ledger_movements)

**Sin esta migración, no se puede:**
- ❌ Crear socios
- ❌ Registrar retiros de socios
- ❌ Ver reportes de retiros de socios
