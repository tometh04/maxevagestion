...
export function DataTableShell({ children, className }: ShellProps) {
  return (
    <div className={cn("overflow-x-auto rounded-lg border border-muted-foreground/80", className)}>
...
export function DataTableHead({ children, className }: ShellProps) {
  return (
    <thead className={cn("bg-ink/60 text-xs font-medium uppercase tracking-wider text-muted-foreground", className)}>
      {children}
    </thead>
  )
}
...
export function DataTableBody({ children, className }: ShellProps) {
  return (
    <tbody className={cn("divide-y divide-muted-foreground/40 bg-ink/40", className)}>
      {children}
    </tbody>
  )
}
...
export function DataTableRow({ children, className, onClick, muted }: RowProps) {
  return (
    <tr
      onClick={onClick}
      className={cn(
        "transition odd:bg-ink/40 even:bg-ink/20 hover:bg-ink/40",
